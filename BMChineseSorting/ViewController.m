...
- (IBAction)reloadData:(UIButton*)sender {
    ...
    if (dataType == 0) {
        //排序 字符串
        [BMChineseSort sortWithArray:[self getStringData] key:nil finish:^(bool isSuccess, NSMutableArray<NSString *> *sectionTitleArr, NSMutableArray<NSMutableArray *> *sortedObjArr) {
            if (isSuccess) {
        ...
        //排序 Person对象
        [BMChineseSort sortWithArray:[self getModelData] key:@"name" finish:^(bool isSuccess, NSMutableArray<NSString *> *sectionTitleArr, NSMutableArray<NSMutableArray *> *sortedObjArr) {
            if (isSuccess) {
                self.firstLetterArray = sectionTitleArr;
                self.sortedModelArr = sortedObjArr;
                [_tableView reloadData];
...
- (IBAction)changeDataTyple:(UISegmentedControl*)sender {
    dataType = sender.selectedSegmentIndex;
}


#pragma mark ----------  模拟数据  ----------
-(NSMutableArray*)getStringData{
    NSMutableArray * provinceArr = @[@"北京",@"天津",@"河北",@"山西",@"内蒙",@"辽宁",@"吉林",@"黑龙江",@"上海",@"江苏",@"浙江",@"安徽",@"福建",@"江西",@"山东",@"河南",@"湖北",@"湖南",@"广东",@"广西",@"海南",@"重庆",@"四川",@"贵州",@"云南",@"西藏",@"陕西",@"甘肃",@"青海",@"宁夏",@"新疆",@"台湾",@"香港",@"澳门",@"沈阳",@"长春",@"abc",@"baba"].mutableCopy;
    return provinceArr;
}
